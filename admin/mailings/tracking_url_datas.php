...
if(isset($_GET['mailing_id']) AND isset($_GET['title_page'])){
$mailing_id = $_GET['mailing_id'];
$urlpage = urlencode($_GET['title_page']);
...
/*** ETAPE 1 - FONCTION DE FABRICATION DES LIENS DES PAGES ***/
      ...
      function AffichageDesLiensDesPages($npages,$messagesparpage,$mailing_id,$urlpage,$visitesparcampagne)
      {
          for($i=1; $i<=$npages; $i++)
        {
          $arrayliensdespages[] = '<a href="tracking_url_datas.php?visitesparcampagne='.$visitesparcampagne.'&amp;messagesparpage='.$messagesparpage.'&amp;page='.$i.'&amp;mailing_id='.$mailing_id.'&amp;title_page='.$urlpage.'">'.$i.'</a>&nbsp;&nbsp;&nbsp;';
        }
...
/*** EXECUTION --> LA FONCTION RETOURNE L'ARRAY CONTENANT LES LIENS DES PAGES. ***/     
      ...
      $liensdespages = AffichageDesLiensDesPages($nbpages,$messagesparpage,$mailing_id,$urlpage,$visitesparcampagne);
...
";
   ...
   $requetegenerale = str_replace(table_clickedurl, $prefix."clickedurl", $requetegenerale);  
            ...
            while($row = $result->fetch())
                {
                    ...
                    <td>$row[dateclic]</td>
                    <td>$row[latence]</td>
                    <td><a href='tracking_url_by_criteria.php?email=$row[email]' title=\"Voir tous les clics enregistrés depuis cette adresse\">$row[email]</td>
...
$smarty->display('admin/mailings/tracking_url_datas.tpl');
Pommo::kill();
?>
