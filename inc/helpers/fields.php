...
class PommoField {
	...
	function delete(&$id) {
		...
		$query = "
			DELETE x, y, z
                        FROM " . $dbo->table['fields'] . " AS x
                        LEFT JOIN " . $dbo->table['subscriber_data'] . " AS y ON x.field_id = y.field_id
                        LEFT JOIN " . $dbo->table['group_rules'] . " AS z ON x.field_id = z.field_id
                        WHERE x.field_id IN(%c)";
